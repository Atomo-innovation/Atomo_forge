...
- Reduced false positives on TV/monitor (orange/yellow)
- Better logging
- Always sends MQTT message (0,0,0,0 when no detection)
"""
...
# ====================== FIRE CONFIG (Improved) ======================
MODEL_PATH = "fire.onnx"
INPUT_SIZE = 640
CONF_THRESH = 0.65          # Increased to reduce false positives
NMS_THRESH = 0.5
...
def yolov3_post_process(input_data):
    ...
    for i in range(3):
        ...
        boxes_list.append(b); classes_list.append(c); scores_list.append(s)
    ...
    def _inference_loop(self):
        ...
        while not self.stop_event.is_set():
            ...
            for pred in outputs:
                ...
                # Simple false positive filter for TV/monitor (orange/yellow areas)
                roi = frame[y1:y2, x1:x2]
                if roi.size > 0:
                    mean_color = cv2.mean(roi)[:3]  # B, G, R
                    if mean_color[2] < 100 and mean_color[1] > 80:  # Too much green/yellow, low red → likely TV
                        continue
            ...
            if fire_detected:
                for i in keep:
                    ...
                    print(f"[{ts}] [{self.cam_name}] 🔥 FIRE DETECTED | {cls_name} | Conf: {conf:.2f} | BBox: {data['bbox']}")

                    email_alerter.alert(self.cam_name, cls_name, conf, [x1, y1, x2, y2], frame)
            else:
                # Send zero when no fire
                data = {
                    "camera": self.cam_name,
                    "class": "none",
                    "confidence": 0.0,
                ...
                if frame_count % 30 == 0:  # Log every 30 frames to avoid spam
                    print(f"[{ts}] [{self.cam_name}] No fire detected → bbox [0,0,0,0] sent")
            ...
            input_data = [np.transpose(input0, (2,3,0,1)), np.transpose(input1, (2,3,0,1)), np.transpose(input2, (2,3,0,1))]
            ...
            if detections:
                print(f"[{ts}] [{self.cam_name}] 👤 Person detected: {len(detections)} | Conf: {detections[0]['confidence']:.2f}")
            elif frame_count % 50 == 0:
                print(f"[{ts}] [{self.cam_name}] No person detected")
...
def main():
    ...
    print(f"Cameras: {len(CAMERAS)} | Fire Conf Threshold: {CONF_THRESH}")
