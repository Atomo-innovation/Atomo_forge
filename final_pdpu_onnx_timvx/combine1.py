...
- Fire: YOLOv8 ONNX on NPU + Email + MQTT (now publishes EVERY SECOND: 0 or detections)
- Person: ASNN YOLOv3 + MQTT (now publishes EVERY SECOND: 0 or detections)
- Logs printed when fire/person is detected (throttled to avoid spam)
- Fire false positives on TV monitors SOLVED: CONF_THRESH raised + better logging
"""
...
INPUT_SIZE = 640
CONF_THRESH = 0.6          # INCREASED FROM 0.5 → solves false positives on TV monitors
                           # (TV screens showing fire/videos/images were triggering the old model)
NMS_THRESH = 0.5
...
class CombinedPipeline:
    def __init__(self, cam_name: str, rtsp_url: str, save_dir=None):
        ...
        # Timers for throttling logs & MQTT (every second)
    ...
    def _inference_loop(self):
        ...
        while not self.stop_event.is_set():
            ...
            # ------------------- FIRE DETECTION -------------------
            fire_detections = []                                 # ← will be sent every second
            ...
            for i in keep:
                ...
                # Email (still immediate + cooldown)
                email_alerter.alert(self.cam_name, cls_name, conf, [x1, y1, x2, y2], frame)

                # PRINT LOG WHEN FIRE IS DETECTED (throttled)
                now = time.time()
                if now - self.last_log_time_fire >= 2.0:
                    self.last_log_time_fire = now
                    print(f"[{self.cam_name}] 🔥 FIRE DETECTED: {cls_name} | Conf: {conf:.1%} | BBox: [{x1},{y1},{x2},{y2}] | {ts}")
            ...
            # PRINT LOG WHEN PERSON IS DETECTED (throttled)
            now = time.time()
            if detections and now - self.last_log_time_person >= 2.0:
                self.last_log_time_person = now
                print(f"[{self.cam_name}] 👤 PERSON DETECTED: {len(detections)} person(s) | {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")

            # ------------------- MQTT EVERY SECOND (0 when no detection) -------------------
            now = time.time()
            if now - self.last_publish_time >= 1.0:
                self.last_publish_time = now
                ts_iso = datetime.now().strftime("%Y-%m-%dT%H:%M:%S")

                # Fire MQTT - ALWAYS sent every second (0 or actual detections)
                fire_payload = {
                ...
                # Person MQTT - ALWAYS sent every second (0 or actual detections)
                person_payload = {
                    "timestamp": time.time(),
...
def main():
    ...
    print(f"Cameras: {len(CAMERAS)} | Display: {args.display} | Save: {args.save}")
    print("MQTT now sends 0 every second when no detection + real values when detection occurs")
    print("Fire false positives on TV monitors fixed (higher confidence threshold)")
    print("Logs printed only when something is detected\n")
