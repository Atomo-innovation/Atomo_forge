...
Uses YOLOv3 person detection and publishes real-time person counts via MQTT.
"""
...
class MQTTManager:
    ...
    def on_connect(self, client, userdata, flags, rc, properties=None):
        self.connected = (rc == 0)
        print(f"[MQTT] {'Connected' if self.connected else f'Error rc={rc}'}")
    ...
    def publish_count(self, camera_id, person_count):
        """Publish real-time person count for a single camera."""
    ...
    def publish_stats(self, stats):
        """Publish aggregated stats across all cameras every 5 s."""
        if not self.connected:
            return
        try:
...
def draw_hud(vis, camera_id, person_count, fps):
    h, w = vis.shape[:2]
    lines = [
        f" {camera_id} ",
        f" FPS   : {fps:5.1f} ",
        f" People: {person_count:3d} ",
    ]
    font    = cv.FONT_HERSHEY_DUPLEX
    fs, thick = 0.5, 1
    pad     = 6
...
if __name__ == '__main__':
    ...
    parser.add_argument("--rtsp_urls",     nargs='+', required=True, help="List of RTSP URLs")
    ...
    # Per-camera current counts
    current_counts = {cid: 0 for cid in camera_ids}
    print("current_count: ", current_counts)
    latest_vis     = {}
    ...
    try:
        while True:
            ...
            current_counts[camera_id] = person_count

            # Publish real-time count for this camera
            mqtt_manager.publish_count(camera_id, person_count)
            ...
            # ---- Display ----
            if args.show:
                vis = draw_detections(orig_img, boxes, scores, classes)
                vis = draw_hud(vis, camera_id, person_count, fps)
                latest_vis[camera_id] = vis
                cv.imshow(camera_id, vis)
                if cv.waitKey(1) & 0xFF in (ord('q'), ord('Q'), 27):
                    break

            # ---- Publish aggregated stats every 5 s ----
            if time.time() - last_stats_time > 5.0:
                agg_total = sum(current_counts.values())
                mqtt_manager.publish_stats({
                    "timestamp":  time.time(),
                    "datetime":   datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
                    "total_people": agg_total,
                    "per_camera": dict(current_counts),
                    "performance": {
                        "fps":               fps,
            ...
            if frame_count % 30 == 0:
                ...
                agg_total = sum(current_counts.values())
                counts_str = " | ".join(f"{c}:{v}" for c, v in current_counts.items())
                print(
                    f"[Stats] FPS: {frame_count / elapsed:.2f} | "
                    f"Total: {agg_total} | {counts_str}"
                )
                frame_count = 0
                start_time  = time.time()
