#!/usr/bin/env python3
"""
Combined Real-Time Fire + Person Detection from Same RTSP Cameras
- Single RTSP connection per camera
- Fire: YOLOv8 ONNX on NPU + Email + MQTT
- Person: ASNN YOLOv3 + MQTT
- Publishes room_position {x,z} (twin floor meters) using same homography as Node server.js
  when camera_calibration.json is present next to this script (or --calibration path).
"""
...
from floor_mapping import FloorMapper, normalize_mqtt_camera_id
...
parser.add_argument(
    ...
    type=str,
    default=None,
    help="camera_calibration.json path (default: same folder as this script)",
)
...
CALIB_PATH = Path(args.calibration) if args.calibration else Path(__file__).resolve().parent / "camera_calibration.json"
floor_mapper = FloorMapper(CALIB_PATH)
...
COLORS_FIRE = {"fire": (0, 60, 255), "FIRE": (150, 150, 150)}
...
MQTT_STATS_TOPIC = "atomo/store/detection_stats"
MQTT_USERNAME = "rajat"
MQTT_PASSWORD = "asdf"
...
mqtt_mgr = MQTTManager(args.mqtt_host, args.mqtt_port)
...
class CombinedPipeline:
    ...
    def _inference_loop(self):
        ...
        mqtt_cam = normalize_mqtt_camera_id(self.cam_name)
        ...
        while not self.stop_event.is_set():
            ...
            for i in keep:
                ...
                room_pos = floor_mapper.bbox_to_floor(self.cam_name, (x1, y1, x2, y2), w, h)
            ...
            data_inf = self.yolov3.nn_inference(
                [img],
                platform="ONNX",
                reorder="2 1 0",
                output_tensor=3,
            ...
            input0 = data_inf[2].reshape(SPAN, LISTSIZE, GRID0, GRID0)
            input1 = data_inf[1].reshape(SPAN, LISTSIZE, GRID1, GRID1)
            input2 = data_inf[0].reshape(SPAN, LISTSIZE, GRID2, GRID2)
            ...
            if boxes_p is not None:
                for idx, (box, score, cl) in enumerate(zip(boxes_p, scores_p, classes_p)):
                    px1 = max(0, int(box[0] * w))
                    py1 = max(0, int(box[1] * h))
                    px2 = min(w, int(box[2] * w))
                    py2 = min(h, int(box[3] * h))

                    room_pos = floor_mapper.bbox_to_floor(self.cam_name, (px1, py1, px2, py2), w, h)
...
# ====================== MAIN ======================
def main():
    print(f"[calibration] {CALIB_PATH} exists={CALIB_PATH.is_file()}")
    mqtt_mgr.start()
    time.sleep(1.5)
    ...
    print(f"Cameras: {len(CAMERAS)} | MQTT: {args.mqtt_host}:{args.mqtt_port}")
    print(f"Display: {args.display} | Save: {args.save}")
    print("Press Ctrl+C to stop\n")

    try:
        while True:
            time.sleep(0.05 if args.display else 0.1)
