...
log "  http://${LAN_IP}/dashboard   ← PHONES: use this (electron.local often fails on Android)"
if [[ "${FORGE_MDNS_ACTIVE:-0}" == "1" ]] || (command -v systemctl >/dev/null 2>&1 && systemctl is-active avahi-daemon &>/dev/null 2>&1); then
  log '  http://electron.local/dashboard   ← PCs only (mDNS)'
else
  log '  http://electron.local   ← run once: npm run lan:setup (PCs; phones use IP above)'
fi
...
if [[ -f "$ROOT/.forge-board" ]] || [[ "${FORGE_BOARD:-}" == "1" ]]; then
  ...
  log 'This laptop: http://electron.local:8443/dashboard  (or http://electron.local if Caddy on :80)'
fi
