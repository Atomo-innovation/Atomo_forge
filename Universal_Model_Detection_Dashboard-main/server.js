...
function spawnInference(sid, session) {
  const { args, model, inputType } = session;
  const send = (payload) => {
  ...
  if (!fs.existsSync(DETECT_SCRIPT)) {
    ...
    send({ type: 'log', level: 'warn', message: `detect.py not found at ${DETECT_SCRIPT} — using simulation mode` });
...
function startSimulation(sid, session) {
  ...
  const interval = setInterval(() => {
    ...
    if (frame % 30 === 0) {
      send({ type: 'log', level: 'info', message: `[SIM] Frame ${frame} | ${dets.length} detections` });
    }
  }, 66); // ~15fps simulation
