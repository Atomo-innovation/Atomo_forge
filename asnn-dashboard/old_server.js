...
app.post('/api/inference/start', (req, res) => {
  ...
  const args     = isPerson
    ? buildPersonArgs({ model, inputValue, objThresh, nmsThresh, logLevel })
  ...
  proc.stdout.on('data', (chunk) => {
    ...
    lines.forEach(line => {
      ...
      try {
        ...
        // Both scripts emit {type:"log",...} for log lines
        // and frame objects (frame, fps, inference_ms, detections, jpeg) for results
        wsend(ws, { type: data.type || 'inference', ...data });
      } catch(e) {
...
// person.py only supports RTSP (LatestFrameReader). If the user picks webcam/video/image
// we warn in the log and fall back to simulation — the server still tries to run it.
function buildPersonArgs({ model, inputValue, objThresh, nmsThresh, logLevel }) {
  // inputValue for rtsp is the URL directly; for webcam it's "usb:0" etc.
  // person.py expects --rtsp <url>; we pass it whatever inputValue is
  const rtspUrl = inputValue || 'rtsp://localhost/stream';
  const args = [
    '--model',        model.nb_path,
    '--library',      model.lib_path,
    '--rtsp',         rtspUrl,
    '--level',        String(logLevel || 0),
    '--json-stream',              // emit dashboard-compatible JSON lines
    '--jpeg-quality', '75',
  ];
